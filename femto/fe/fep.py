...
Notes:
    Currently the implementation here is very simple and only assumes morphing
    full ligands to ideal states. This is sufficient for the current use case
...
def _apply_nonbonded_lambdas(
    ...
    if not config.scale_charges and not config.scale_vdw:
        return (force,)

    if config.ligands_can_interact:
        raise NotImplementedError("ligands_can_interact=True is not yet implemented")
